...
class INSTRUMENT(object):
    def __init__(self,letter, **kwargs):
        self.com = {'label':'', 'Ranges':[], 'measure_seperation':'0', 'NoError':'','reset':'','status':'','init':'','Make_Safe':'', 'error':'', 'SettleTime':'0', 'DCVRange':'', 'SetVoltage':'', 'operate':'', 'standby':'','MeasureSetup':'','SingleMsmntSetup':''} #command dictionary
    ...
    def make_safe(self):
        ...
        """
        commandstring = 'self.' + self.com['label'] + ".write('" + self.com['Make_Safe'] +"')"
        execstring = 'self.try_command(' + '"' + commandstring + '"' + ", 'Failing to set initial state of " + self.com['label'] + "', 'ex',"+str(self.com['SettleTime'])+")"
    ...
    def set_DCrange(self, value):
        """
        DC range setting could include multiple lines, so put 'value' in place of '[]'
        """
        line = str(self.com['DCVRange'])
        line = line.replace("$",str(value))
        commandstring = 'self.' + self.com['label'] + ".write('" + line +"')"
        commandstring = '"' + commandstring + '"'
        execstring = "self.try_command(" + commandstring + ", 'Failing to set DCV range " + self.com['label'] + "', 'ex',"+str(self.com['SettleTime'])+")"
        return execstring
    ...
    def set_DCvalue(self, value):
        """
        DC range setting could have the actuall DC value in the middle of a command string, eg "OUT 10V" so read
        command string, and replace $ with the voltage setting. Return the exec string
        """
        line = str(self.com['SetVoltage'])
        line = line.replace("$",str(value))
        commandstring = 'self.' + self.com['label'] + ".write('" + line +  "')"
        commandstring = '"' + commandstring + '"'
        execstring = "self.try_command(" + commandstring + ", 'Failing to set NPLC " + self.com['label'] + "', 'ex',"+str(self.com['SettleTime'])+")"
        return execstring       

    def Operate(self):
        """
        generates exec compatible query to operate the instrument, as specified by the user
        """
        commandstring = 'self.' + self.com['label'] + ".write('" + self.com['operate'] +"')" #repr(nominal) to str(nominal)
        commandstring = '"' + commandstring + '"'
        execstring = "self.try_command(" + commandstring + ", 'Failing to set DC volts " + self.com['label'] + "', 'ex',"+str(self.com['SettleTime'])+")"
        return execstring

    def Standby(self):
        """
        generates exec compatible query for standing by
        """
        commandstring = 'self.' + self.com['label'] + ".write('" + self.com['standby']+ "')" #repr(nominal) to str(nominal)
        commandstring = '"' + commandstring + '"'
        execstring = "self.try_command(" + commandstring + ", 'Failing to set DC volts " + self.com['label'] + "', 'ex',"+str(self.com['SettleTime'])+")"
        return execstring
